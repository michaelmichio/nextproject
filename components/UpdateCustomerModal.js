...
export default function UpdateCustomerModal({ isVisible, onClose, customerData }) {
    ...
    async function updateCustomerHandler(e) {
        e.preventDefault();

        setStatus('loading');

        const updateCustomerReq = await fetch('/api/customer/update/' + customerData.id, {
            method: 'PUT',
        ...
        const updateCustomerRes = await updateCustomerReq.json();
                ...
                <div role="alert" className="container mx-auto w-11/12 md:w-2/3 max-w-lg">
                    <div className="relative py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400">
                        <form onSubmit={updateCustomerHandler.bind(this)} action="">
                            ...
                            <input required onChange={fieldHandler.bind(this)} name="name" id="name" className="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border" defaultValue={customerData.name} />
                            <label htmlFor="address" className="text-gray-800 text-sm font-bold leading-tight tracking-normal">Alamat</label>
                            <input required onChange={fieldHandler.bind(this)} name="address" id="address" className="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border" defaultValue={customerData.address} />
                            <label htmlFor="phone" className="text-gray-800 text-sm font-bold leading-tight tracking-normal">Nomor Telepon</label>
                            <input required onChange={fieldHandler.bind(this)} name="phone" id="phone" className="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border" defaultValue={customerData.phone} />
