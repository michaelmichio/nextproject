...
export default function CreateItemModal({ isVisible, onClose }) {
    ...
    async function createItemHandler(e) {
        ...
        const createItemReq = await fetch('/api/item/create', {
            method: 'POST',
        ...
        if(!createItemReq.ok) return setStatus('error' + createItemReq.status);

        const createItemRes = await createItemReq.json();
            ...
            <div className="h-full py-12 bg-black bg-opacity-30 transition duration-150 ease-in-out z-10 absolute top-0 right-0 bottom-0 left-0" id="modal">
                <div role="alert" className="container mx-auto w-11/12 md:w-2/3 max-w-lg">
                    <div className="relative py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400">
                        <form onSubmit={createItemHandler.bind(this)} action="">
                            <label htmlFor="code" className="text-gray-800 text-sm font-bold leading-tight tracking-normal">Kode</label>
                            <input required onChange={fieldHandler.bind(this)} name="code" id="code" className="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border" placeholder="" />
                            <label htmlFor="name" className="text-gray-800 text-sm font-bold leading-tight tracking-normal">Nama</label>
                            <input required onChange={fieldHandler.bind(this)} name="name" id="name" className="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border" placeholder="" />
                            <label htmlFor="price" className="text-gray-800 text-sm font-bold leading-tight tracking-normal">Harga</label>
                            <input required onChange={fieldHandler.bind(this)} name="price" id="price" className="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border" placeholder="" />
                            <label htmlFor="stock" className="text-gray-800 text-sm font-bold leading-tight tracking-normal">Jumlah</label>
                            <input required onChange={fieldHandler.bind(this)} name="stock" id="stock" className="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border" placeholder="" />
